...
def start_game():
    ...
    setPathToFile()
    setIndexFromUser()
    print(Const.starting_faze)

    # choose word according to the user
    secret_word = choose_word(Const.PATH_TO_FILE, Const.INDEX_FROM_USER)
...
def ascii_background():
    ...
    print(Const.HANGMAN_ASCII_ART, Const.MAX_TRIES)

def setPathToFile():
    """Get path from the user & check if the path exists"""
    Const.PATH_TO_FILE = str(input("Please enter path to file: "))
    while (not os.path.exists(Const.PATH_TO_FILE)):
...
def setIndexFromUser():
    ...
    Const.INDEX_FROM_USER = int(temp_index)
...
def manage_game(secret_word, old_letters_guessed):
    ...
    while (Const.MAX_TRIES > num_of_tries):
        ...
        if(try_update_letter_guessed(letter_guessed.lower(), old_letters_guessed)):
            ...
            if letter_guessed.lower() in secret_word:
                ...
                print(Const.HANGMAN_PHOTOS[num_of_tries])
                show_hidden_word(secret_word, old_letters_guessed)
...
def try_update_letter_guessed(letter_guessed, old_letters_guessed):
    ...
    if not check_valid_input(letter_guessed.lower(), old_letters_guessed):
        print('X')
        print("Your oldest guesses are: ", sorted(old_letters_guessed))
        return False
    else:
        return True
